...
// GET /api/public/catalog/models/:brandSlug
async function handler(req, res) {
  if (req.method !== 'GET') return res.status(405).json({ error: 'Method not allowed' });

  const { slug } = req.query; // must be EXACT brand slug returned by /catalog/devices
  const base = (process.env.REUSELY_BASE || 'https://api-us.reusely.com/api/v2').replace(/\/+$/,'');
  const url  = `${base}/public/catalog/model/${encodeURIComponent(slug)}`;
