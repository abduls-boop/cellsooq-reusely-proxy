...
export default withCors(async function handler(req, res) {
  ...
  const { slug } = req.query; // model slug, e.g. 'iphone-11'
  const url = `${(process.env.REUSELY_BASE || 'https://api-us.reusely.com/api/v2').replace(/\/+$/,'')}/public/catalog/model/${encodeURIComponent(slug)}/products`;
