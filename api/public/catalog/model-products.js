// api/public/catalog/model-products.js
...
async function handler(req, res) {
  ...
  const base = (process.env.REUSELY_BASE || 'https://api-us.reusely.com/api/v2')
    .replace(/\/+$/, '');
  const slug = (req.query.slug || '').toString().trim().toLowerCase();

  if (!slug) {
    return res.status(400).json({ error: 'Missing slug (e.g. ?slug=iphone-14)' });
  }
